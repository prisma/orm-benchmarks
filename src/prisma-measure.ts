...
async function main() {
  ...
  await measure('prisma-findFirstOrThrow-1-level-nesting', prisma.customer.findFirstOrThrow({
    include: {
      orders: true,
    },
  }));
  ...
  await measure('prisma-findUniqueOrThrow', prisma.customer.findUniqueOrThrow({
    where: { id: 1 },
  }));

  await measure('prisma-findUniqueOrThrow-1-level-nesting', prisma.customer.findUniqueOrThrow({
    where: { id: 1 },
    include: {
      orders: true,
    },
